      ...
      <div v-show="!isShowTable">
        ...
        <el-table style="width: 100%;margin: 20px 0px;" border :data="attrInfo.attrValueList">
          ...
          <el-table-column width="width" prop="prop" label="属性值名称">
            <template slot-scope="{row,$index}">
              <el-input placeholder="请输入属性值名称" v-model="row.valueName" size="mini" v-if="row.flag" @blur="toLook(row)"
  ...
  methods: {
    ...
    addAttrValue() {
      this.attrInfo.attrValueList.push({
        //id如果是新增就是undefined，如果是修改就把已有的id带上
    ...
    async addOrUpdateAttr() {
      //判断数组里是否有属性值为空的
      let permit = this.attrInfo.attrValueList.every(item => {
